...
\end{enumerate}
This research contributes a rigorous, multi-framework benchmark that provides the first (to the my knowledge) clear performance and methodological analysis of K-Means on MLX, offering practical guidance to practitioners on Apple Silicon.
...
\begin{itemize}
    \item \textbf{MLX:} Our test candidate, using the `mlx.core` API.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Fit Time (Seconds):} Wall-clock time to run the `fit()` method, averaged over 20 rounds.
        \item \textbf{Peak Memory (MB):} The peak memory usage recorded during the `fit()` process.
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    \item \textbf{No Boolean Indexing:} MLX (at the time of writing) lacks direct support for NumPy's `X[labels == k]`. This required a work-around using mask-based operations to recalculate centroids.
    \item \textbf{Limited Random API:} MLX does not have `np.random.choice`; therefore, `mx.random.permutation()` followed by a slice was used to achieve random centroid initialization.
...
\begin{figure*}[htbp]
    ...
    \caption{K-Means Implementation Comparison on Credit Card Data.}
...
The most significant finding is MLX's exceptional memory efficiency. As shown in Table \ref{tab:h2-results}, MLX and PyTorch used $\approx$0.13MB of peak memory. This was \textbf{$\approx$34x less memory} than `scikit-learn` (4.40MB) and \textbf{$\approx$108x less memory} than the `NumPy` baseline (14.16MB). This strongly supports H2 and highlights the key advantage of modern frameworks leveraging unified memory.

\textbf{Runtime (H2 Nuanced):}
For this small dataset, `NumPy` (0.025s) and `MLX` (0.046s) were the fastest, as they have low overhead. The more complex frameworks, `scikit-learn` (0.247s) and `PyTorch` (0.165s), were slower due to their initialization costs.
...
\begin{enumerate}
    ...
    \item For small-to-medium datasets on Apple Silicon where \textbf{memory efficiency is critical}, MLX is a better choice than `scikit-learn`. `sklearn`'s 30-40x higher memory footprint can be a significant liability in memory-constrained environments.
\end{enumerate}
...
Kaggle (2020). \textit{Credit Card Customers}. \href{https://www.kaggle.com/datasets/sakshigoyal7/credit-card-customers}{https://kaggle.com/datasets/sakshigoyal7/credit-card-customers}
