...
print("KMeans (NumPy) benchmarking...")
...
kmeans_np = KMeans_NP(n_clusters=2, max_iters=500, random_state=42)
current_pre_fit, peak_pre_fit = tracemalloc.get_traced_memory()
labels = kmeans_np.fit_predict(X)
fit_time = time.time() - start_time
current_post_fit, peak_post_fit = tracemalloc.get_traced_memory()
tracemalloc.stop()

print(f"KMeans (NumPy) fit time: {fit_time:.4f} seconds")
print(
    f"KMeans (NumPy) pre-fit memory usage: {current_pre_fit / 10**6:.4f} MB; peak: {peak_pre_fit / 10**6:.4f} MB"
)
print(
    f"KMeans (NumPy) current memory usage: {current_post_fit / 10**6:.4f} MB; peak: {peak_post_fit / 10**6:.4f} MB"
)
...
print(f"KMeans (NumPy) Adjusted Rand Index: {ari_score:.4f}")
print(f"KMeans (NumPy) Silhouette Score: {sil_score:.4f}")
print(f"KMeans (NumPy) Normalized Mutual Information: {nmi_score:.4f}")
...
np.savez("src/results/kmeans_np_benchmark_scores.npz", **scores)
