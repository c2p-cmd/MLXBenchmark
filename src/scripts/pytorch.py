...
print("KMeans (PyTorch) benchmarking...")
...
kmeans_torch = KMeans_Torch(n_clusters=2, max_iters=500, random_state=42)
current_pre_fit, peak_pre_fit = tracemalloc.get_traced_memory()
labels = kmeans_torch.fit_predict(X)
...
print(f"KMeans (PyTorch) fit time: {fit_time:.4f} seconds")
print(
    f"KMeans (PyTorch) pre-fit memory usage: {current_pre_fit / 10**6:.4f} MB; peak: {peak_pre_fit / 10**6:.4f} MB"
)
print(
    f"KMeans (PyTorch) current memory usage: {current_post_fit / 10**6:.4f} MB; peak: {peak_post_fit / 10**6:.4f} MB"
)
...
print(f"KMeans (PyTorch) Adjusted Rand Index: {ari_score:.4f}")
print(f"KMeans (PyTorch) Silhouette Score: {sil_score:.4f}")
print(f"KMeans (PyTorch) Normalized Mutual Information: {nmi_score:.4f}")
...
np.savez("src/results/kmeans_torch_benchmark_scores.npz", **scores)
